...
class PriceComparisonStrategy: ComparisonStrategy {
     ...
     */
    override fun compare(rule: ComparisonRuleDomain, products: List<ProductDomain>): ComparisonAggregateRoot {
        val productsWithFinalPrice = products.map { product ->
            ComparisonDTO(
                info = product,
        ...
        val sortedProducts = if (lowerIsBetter) {
        ...
        val ranking = sortedProducts.mapIndexed { index, product ->
            ComparisonRankingAg(
        ...
        val averagePrice = sortedProducts.map { it.finalPrice }.let { prices ->
            if (prices.isEmpty()) BigDecimal.ZERO
            else prices.reduce { acc, price -> acc + price } / prices.size.toBigDecimal()
        }

        return ComparisonAggregateRoot(
            strategy = this.type.name,
            winner = ComparisonWinnerAg(
                product = winner.info,
                reason = REASON
            ),
            ranking = ranking,
            rule = rule,
            details = ComparisonDetailAg(
                average = ComparisonAverageVO(
                    value = averagePrice
                )
            )
        )
